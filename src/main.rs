...
fn main() {
    for l in io::stdin().lock().lines() { if l.map_or(true, |s| s.trim().is_empty()) { break; } }

    let mut args = std::env::args().skip(1);
    let mode = args.next().unwrap_or_default();
    let dialed: String = args.next().unwrap_or_default().chars().filter(|c| c.is_ascii_digit()).collect();
    let dialed = match dialed.len() {6 => format!("73843{dialed}"),10 => format!("7{dialed}"),11 if dialed.starts_with('8') => format!("7{}", &dialed[1..]), _ => dialed};
    let caller = args.next().unwrap_or_default();

    let found = if mode=="in" {
        if let Some(&(_,ext)) = MAPPINGS.iter().find(|&&(n,_)| n==dialed) { println!(r#"SET VARIABLE DIAL_TARGET "{ext}""#); true } else { false }
    } else {
        if let Some(&(trunk,_)) = MAPPINGS.iter().find(|&&(t,e)| t==caller || e==caller) { 
